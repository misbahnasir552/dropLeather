...
// // import { storeFields } from '@/utils/fields/AddMerchantAccount/addStoreFields/addStoreFields';
...
// //   addStoreSchema,
// // } from '@/validations/admin/registerationQueue/decision/addMerchantAccount';

// import { addStoreInitialValues, addStoreSchema } from '@/validations/admin/registerationQueue/addMerchantAccount';
...
//   const [newStataicFeilds, setNewStataicFeilds] = useState<any>([]);
//   const [apierror, setApierror] = useState('');
//   const { apiSecret, jwt, managerMobile } = adminData;
...
//       // Case 1: If 'show' is not present or has multiple values (length > 1), return the field as is
//       if (!field.show || field.show.length > 1) {
//         return field;
//       }

//       // Case 2: If 'show' has only one value, check if it matches 'Online' or 'Retail'
//       if (
//         field.show.length === 1 &&
//         checkboxValue &&
//         checkboxValue?.length == 1
//       ) {
//         if (checkboxValue && checkboxValue.includes(field.show[0])) {
//           return field; // If show matches 'Online' or 'Retail', return the field
//         }
//       } else {
//         return field;
//       }

//       // If show has only one value and does not match 'Online' or 'Retail', exclude the field.
//       return null; // or some other logic to exclude the field
//     });

//     // Filter out null values (if you choose to exclude fields that don't match the conditions above)
//     const finalStoreFields = updatedStoreFields.filter(
//       (field) => field !== null,
//     );

//     // Now update the 'region' and 'category' fields with the provided options
//     const finalUpdatedFields = finalStoreFields.map((field: any) => {
...
//         return {
//           ...field,
//           options: storeCategories, // Set the fetched categories as options for 'category'
//         };
//       }
//       return field;
//     });

//     setNewStoreFields(finalUpdatedFields);
//   };
...
//     getRegions();
//     getDynamicFields();
//   }, [regions.length, storeCategories.length, checkboxValue]);

//   // useEffect(() => {

//   // }, [checkboxValue, regions.length, storeCategories.length]);

//   // const finalStoresvalue = getDynamicFields()

//   // console.log('finalStoresvalue===', finalStoresvalue)
...
//     const categoryLabel: any = storeCategories.find(
//       (store: any) => store.value === values.category,
//     );
//     setAddStoresValues((prevStores) => [
//       ...prevStores,
//       { ...values, category: categoryLabel?.label },
//     ]);

//     const result = [
//       newStoreFields.reduce((acc: any, field: any) => {
//         // Check if the field name is 'storeType' and handle it as a comma-separated string
//         if (field.name === 'storeType') {
//           acc[field.label] = values[field.name].join(', '); // Join the array into a comma-separated string
//         } else if (field.name === 'category') {
//           acc[field.label] = categoryLabel.label;
//         } else {
//           acc[field.label] = values[field.name] || ''; // For other fields, just assign the value or an empty string
//         }
//         return acc;
//       }, {}),
//     ];

//     setNewStataicFeilds(result);
