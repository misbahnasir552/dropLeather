...
// import { searchBulkInitialValues, searchBulkSchema } from '@/validations/merchant/merchant-portal/reversal-module/reversal-bulk-batch';
// import HeaderWrapper from '@/components/UI/Wrappers/HeaderWrapper';
// import Pagination from '@/components/Pagination/Pagination';
// import * as XLSX from 'xlsx';

// function BulkReversalReport() {
...
//         },);
//       if (response?.data) {
//         const formattedRoles = response?.data?.reversalRecords?.map((file: any) => ({
//           value: file?.fileName,
//           label: file?.fileName,
//         }));
...
//       status: item.status,
//       failureReason: item?.responseCode === "000" ? item?.responseDescription:"N/A"
//     }));
//   };
...
//     XLSX?.utils?.book_append_sheet(wb, ws, 'Bulk Reversal Report');

//     // Generate an Excel file and download it
//     XLSX.writeFile(wb, 'bulk_reversal_report.xlsx');
//   };
...
//           : true;
//         const matchesStatus = values.status
//           ? record.status
//           : true;
...
// export default BulkReversalReport;
...
function BulkReversalReport() {
  ...
  const generatebulkTableData = (response: any) => {
    return response?.map((item: any, index: any) => ({
      ...
      status: item.status,
      failureReason:
        item?.responseCode === '000' ? item?.responseDescription : 'N/A',
    }));
  };
  ...
  return (
    <>
      <div className="flex flex-col gap-10 py-12">
        <HeaderWrapper heading="Bulk Reversal Report" />
        <div className="flex flex-col gap-4">
          <Formik
            initialValues={searchBulkInitialValues}
            validationSchema={searchBulkSchema}
